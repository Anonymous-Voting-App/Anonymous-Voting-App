...
import { KeyboardArrowDown, AddCircleOutline, RemoveCircleOutline } from '@mui/icons-material';
...
function QuestionComponent(props: any) {
    ...
    const removeOption = (index: number) => {
        const newOptions = quesOptions.filter((option, i) => { return i !== index });
    ...
    return (
        <>
            <div className='question-wrapper' >
                <FormControl className='type-dropdown' sx={{ m: 1, minWidth: 120 }} size="small">
                    <InputLabel id="demo-select-small">Question type</InputLabel>
                    <Select
                ...
                <TextField sx={{ minWidth: inputWidth }}
                    autoComplete='off'
                                ...
                                InputProps={{
                                    startAdornment: (
                                        <InputAdornment position="start">
                                            <RemoveCircleOutline onClick={() => removeOption(index)} />
                                        </InputAdornment>
                                    )
                                }}
                                autoComplete='off'
                                variant="outlined"
                                onChange={(event) => onOptionInput(index, event.target.value)}
                                key={index} type="text" value={quesOption}
                            />
                        ))}
                        <Button className='add-option-btn' onClick={addOption} sx={{ mt: "4.5rem", width: 200 }} variant="outlined"><AddCircleOutline />Add a new option</Button>
                    </>
                    : null}
            </div>
        </>
