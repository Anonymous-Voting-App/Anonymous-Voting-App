...
export default class User {
     ...
     */
     ...
     async existsInDatabase(  ): Promise<boolean> {
        ...
        if ( this.id(  ).length > 0 || 
            ...
            result = ( await this._database.user.findFirst( this.findSelfInDatabaseQuery(  ) ) ) !== null;
     ...
     */
     ...
     async createNewInDatabase(  ): Promise<void> {
        ...
        var data = await this._database.user.create( { data: this.newDatabaseObject(  ) } );
    ...
    async loadFromDatabase(  ): Promise<void> {
        ...
        pre("either id, ip, cookie or accountId is set", 
            this.id(  ).length > 0 || 
            this.ip(  ).length > 0 || 
            this.cookie(  ).length > 0 || 
            this.accountId(  ).length > 0);

        var data = await this._database.user.findFirst( this.findSelfInDatabaseQuery(  ) );
